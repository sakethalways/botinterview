...
import SetupFormComp from "./components/SetupForm"
import FeedbackReportComp from "./components/FeedbackReport"
import AudioVisualizer from "./components/AudioVisualizer"
import CountdownOverlay from "./components/CountdownOverlay"
import { type InterviewConfig, InterviewState, type ChatMessage, type FeedbackData } from "./types"
...
// Debounced transcript hook - Batches rapid turn completions to prevent render thrashing
const useDebouncedTranscript = (onFlush: (messages: ChatMessage[]) => void, delayMs = 100) => {
  const batchRef = useRef<ChatMessage[]>([])
  const timeoutRef = useRef<NodeJS.Timeout | null>(null)

  const addMessagesToBatch = useCallback(
    (messages: ChatMessage[]) => {
      batchRef.current.push(...messages)
  ...
  return { addMessagesToBatch, flushBatch }
}

// Virtualized transcript component - Only renders visible messages to prevent lag with long conversations
const VirtualizedTranscript = React.memo(
  ({
    messages,
    currentInputTrans,
    currentOutputTrans,
    ...
    messages: ChatMessage[]
    currentInputTrans: string
    currentOutputTrans: string
    ...
    const handleScroll = useCallback(() => {
      ...
      const start = Math.max(0, Math.floor(scrollTop / messageHeight) - 5)
      const end = Math.ceil((scrollTop + containerHeight) / messageHeight) + 5

      setVisibleRange({ start, end })
    }, [containerHeight, messageHeight])

    const visibleMessages = messages.slice(visibleRange.start, visibleRange.end)
    const offsetY = visibleRange.start * messageHeight

    return (
      <div ref={containerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
        <div style={{ height: visibleRange.start * messageHeight }} />

        {visibleMessages.map((msg, idx) => {
          const actualIdx = visibleRange.start + idx
          return (
            <div key={actualIdx} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"} mb-3`}>
              <div
                className={`max-w-[85%] sm:max-w-[80%] p-3 rounded-xl text-sm ${
                  msg.role === "user" ? "bg-blue-600/20 text-blue-100" : "bg-slate-700/40 text-slate-200"
                }`}
              >
                {msg.text}
              </div>
        ...
        {(currentInputTrans || currentOutputTrans) && (
          <>
            {currentInputTrans && (
              <div className="flex justify-end opacity-80 mb-3">
            ...
            {currentOutputTrans && (
              <div className="flex justify-start opacity-80 mb-3">
...
const App: React.FC = () => {
  ...
  const { addMessagesToBatch, flushBatch } = useDebouncedTranscript(
    (batchedMessages) => {
      startTransition(() => {
        setTranscripts((prev) => [...prev, ...batchedMessages])
      })
    },
    100, // Debounce for 100ms - batches rapid updates
  )
  ...
  const stopAllMedia = async () => {
    ...
    if (audioContextRef.current && audioContextRef.current.state !== "closed") {
      try {
        ...
        console.warn("Input Ctx close warning", e)
      }
    ...
    if (outputAudioContextRef.current && outputAudioContextRef.current.state !== "closed") {
      try {
        ...
        console.warn("Output Ctx close warning", e)
      }
      ...
      const connection = ai.live.connect({
        ...
        callbacks: {
          onopen: () => {
            ...
            if (processorRef.current) {
              processorRef.current.onaudioprocess = (e) => {
                ...
                // --- DOWNSAMPLING LOGIC (48kHz -> 16kHz) ---
                // This reduces the data payload by ~66%, drastically reducing network latency
                // and speeding up the text transcription response on the UI.
                ...
                // Create blob with CORRECT 16kHz header
          ...
          onmessage: async (msg: LiveServerMessage) => {
            ...
            // --- 1. HANDLE TEXT IMMEDIATELY WITH FLUSHED UPDATES (Zero UI Latency) ---
            ...
            if (msg.serverContent?.turnComplete) {
              ...
              if (input.trim() || output.trim()) {
                // This prevents React from re-rendering massive transcript arrays on every turn
                addMessagesToBatch([
              ...
              // Clear state with flushed updates for instant clear
              currentInputRef.current = ""
              currentOutputRef.current = ""
              flushSync(() => {
                setCurrentInputTrans("")
  ...
  const handleEndInterview = async () => {
    ...
    const finalTranscript =
      fullTranscriptRef.current +
      (currentInputRef.current ? `\nCandidate: ${currentInputRef.current}` : "") +
      (currentOutputRef.current ? `\nInterviewer: ${currentOutputRef.current}` : "")
      ...
      try {
        ...
        feedbackData = {
          ...
          improvements: Array.isArray(parsed.improvements)
            ? parsed.improvements
            : ["No specific improvements detected"],
          metrics: {
  ...
  const handleHardReset = async () => {
    ...
    try {
      ...
      // Ensure batch is flushed on reset
      flushBatch()
      ...
      <div className="relative z-10 container mx-auto px-4 py-4 md:py-6 min-h-screen flex flex-col">
        <header className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => setState(InterviewState.IDLE)}>
            ...
            <span className="font-bold text-lg md:text-xl tracking-tight hidden xs:block">InterviewFlow AI</span>
        ...
        <main className="flex-1 flex flex-col items-center justify-center relative w-full">
          {state === InterviewState.IDLE && <SetupFormComp onStart={startInterview} isLoading={false} />}

          {(state === InterviewState.CONNECTING || (state === InterviewState.ACTIVE && showCountdown)) &&
            !showCountdown && (
            ...
            <div className="w-full max-w-4xl animate-fade-in flex flex-col gap-4 md:gap-6">
              <div className="relative w-full h-64 sm:h-80 bg-slate-800/50 rounded-3xl overflow-hidden border border-slate-700/50 shadow-2xl">
                ...
                <div className="absolute bottom-4 left-4 sm:bottom-6 sm:left-6 max-w-[65%] sm:max-w-[70%]">
                  ...
                  <div className="text-slate-400 text-sm flex flex-wrap items-center gap-2">
                    ...
                    {config?.context && (
                      <span className="text-blue-400 text-[10px] sm:text-xs px-2 py-0.5 bg-blue-500/10 rounded border border-blue-500/20 whitespace-nowrap">
                        Context
                      </span>
                    )}
                    {config?.resumeText && (
                      <span className="text-green-400 text-[10px] sm:text-xs px-2 py-0.5 bg-green-500/10 rounded border border-green-500/20 whitespace-nowrap">
                        Resume
                      </span>
                    )}
                    {config?.enableGestures && (
                      <span className="text-indigo-400 text-[10px] sm:text-xs px-2 py-0.5 bg-indigo-500/10 rounded border border-indigo-500/20 whitespace-nowrap flex items-center gap-1">
                        <VideoCameraIcon className="w-3 h-3" /> Video Analysis
                      </span>
                    )}
                  </div>
                </div>
              ...
              <div className="w-full glass-panel bg-slate-900/80 rounded-2xl p-4 border-t border-slate-700/50 flex flex-col h-64">
                ...
                <VirtualizedTranscript
                  ...
                  currentInputTrans={currentInputTrans}
                  currentOutputTrans={currentOutputTrans}
                  containerHeight={256} // Corresponds to h-64 in Tailwind CSS
                  messageHeight={80} // Estimate height of a message, adjust as needed
          ...
          {state === InterviewState.ANALYZING && (
            <div className="text-center animate-fade-in max-w-md px-4">
              ...
              <p className="text-slate-400 h-6 transition-all duration-500 ease-in-out text-sm md:text-base">
              ...
              <div className="bg-red-500/10 p-6 md:p-8 rounded-3xl border border-red-500/20 text-center shadow-2xl">
                ...
                <div className="mt-6 pt-6 border-t border-red-500/20">
                  <p className="text-xs text-slate-400 mb-3">
                    Stuck in a loop? Use the reset button to clear internal connections.
                  </p>
                  <button
                    onClick={handleHardReset}
        ...
        <footer className="mt-auto pt-6 border-t border-slate-800 text-center text-slate-500 text-xs">
          <p>© 2024 InterviewFlow AI • Powered by Gemini 1.5 Flash</p>
        </footer>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-lg w-full p-6 shadow-2xl relative">
            ...
            <div className="space-y-4 text-slate-300 text-sm leading-relaxed">
              <p>
                <strong className="text-white">Real-time Analysis:</strong> We stream audio directly to Gemini 1.5 Flash
                using the low-latency Live API over WebSockets.
              </p>

              <p>
                <strong className="text-white">Context Awareness:</strong> Your "Additional Context" inputs are injected
                into the system prompt, forcing the model to adapt its persona using few-shot prompting techniques.
              </p>

              <p>
                <strong className="text-white">Deep Analysis:</strong> After the call, the full transcript is sent back
                to Gemini. We use a specialized prompt to extract structured JSON metrics (0-100 scoring) and actionable
                feedback.
              </p>
